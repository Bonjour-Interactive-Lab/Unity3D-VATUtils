...
float2 VAT_uvPositionControlled(float2 uvIndex, int numOfFrames, float speed, float time, float2 paddedRatio, bool isTimeControlled)
{
    float2 uvPosition;
    float FPS = 24.0;
    float FPS_div_Frames = FPS / numOfFrames;
    float timeInFrames = (isTimeControlled) ? time : frac(speed * time);

    timeInFrames = ceil(timeInFrames * (numOfFrames-1)); //-1 in order to keep animation at last frame when time == 1
    timeInFrames /= numOfFrames;
    timeInFrames += (1/numOfFrames);
...
{
    float2 uvPosition = VAT_uvPositionControlled(uvIndex, numOfFrames, speed, time, paddedRatio, isTimeControlled);
    ...
    outPosition = (restPosition + texturePos) * globalScale;
